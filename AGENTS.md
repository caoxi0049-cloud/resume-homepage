...
- 如配置飞书应用环境变量，问答还会同步写入飞书多维表格「AI简历助手问答记录-应用写入版」：https://my.feishu.cn/base/NXrPbAWwkaPT0UswkSbcNkbfnqd

飞书问答记录环境变量：

- `LARK_APP_ID`
- `LARK_APP_SECRET`
- `LARK_LOG_BASE_TOKEN=NXrPbAWwkaPT0UswkSbcNkbfnqd`（可选，代码已有默认值）
- `LARK_LOG_TABLE_ID=tblnbDHOwmx3oevO`（可选，代码已有默认值）
